                      ...
                      uint8_t *buf, size_t buflen)
{
  ...
  if(result == CURLE_AGAIN) {
    ...
    DEBUGF(infof(data, "Hyper: READ IO_PENDING\n"));
                       ...
                       const uint8_t *buf, size_t buflen)
{
  ...
  if(result == CURLE_AGAIN) {
    ...
    DEBUGF(infof(data, "Hyper: WRITE IO_PENDING\n"));
    return HYPER_IO_PENDING;
  }
  else if(result)
    return HYPER_IO_ERROR;
                             ...
                             size_t value_len)
{
  ...
  infof(data, "header name: %d bytes\n", (int)name_len);
  fprintf(stderr, "%.*s: %.*s\n",
                            ...
                            int select_res)
{
  ...
  *done = FALSE;
  ...
  if(!task) {
    fprintf(stderr, "hyper_executor_poll returned NULL\n");
    *didwhat = KEEP_RECV;
    return CURLE_OK;
  }
  else if(h->init) {
  ...
  infof(data, "h->init was not TRUE\n");
  h->init = TRUE;
  *didwhat = KEEP_RECV;
  resp = hyper_task_value(task);
  if(!resp)
    goto error;
  infof(data, "resp is %p, task is %p\n", resp, task);
